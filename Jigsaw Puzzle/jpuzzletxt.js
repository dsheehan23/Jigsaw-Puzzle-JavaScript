...
function init() {
    ...
    // Throw all grid elements in an array, all piece elements in an array
    for (var i = 0; i < allElem.length; i++) {
        if (allElem[i].className == "grid") grids.push(allElem[i]);
        if (allElem[i].className == "pieces") pieces.push(allElem[i]);
    }

    // loop through pieces array and assign a picture to each one randomly
    //Random array returns an array of random integers
    // Each background piece is set to a random picture based on elements on the array
    // write loops to set position and size style properties of pieces and grids
    ...
    for (var i = 0; i < pieces.length; i++) {
        ...
        // add the mousegrab event to each piece
    ...
    // when a key is pressed down, call the keyGrab function
    // Initialize current key piece
...
// set toggle mode function for space bar, set key commands depending on mode
function keyGrab(e) {
...
// move the current piece depending on the parameters
function keyMove(moveX, moveY) {
    ...
    // highlight the grid place the key is over
    highlightGrid(keyPiece);
}

// Select a different piece by using arrows, but dont go out of array
...
// switch between select and move modes when space bar is pressed
function toggleMode() {
    // switch the mode if the drop isnt on another piece
    if (dropValid(keyPiece)) selectMode = !selectMode;
    var modeBox = document.getElementById("keyMode");

    if (selectMode) {
        keyPiece.style.borderColor = "red";
        // align the previous piece to its grid space 
        alignPiece(keyPiece);
        ...
        // make the piece being move the dominant top piece
        maxZ++;
...
// determine that you are not dropping an object onto another one
...
// when dropping a piece onto a grid, align it with the grid spot
...
// this function highlights the grid that the piece hovers over
...
// make the grabbed piece dominant, get positions of pointer and piece, call other mouse functions
function mouseGrab(e) {
    ...
    maxZ++;
    mousePiece.style.zIndex = maxZ; // place piece above other objects
    mousePiece.style.cursor = "move";

    // position of pointer at moment mouse is clicked down
...
// move the piece, its top and left spots are relative to the current spot
// of the pointer 
function mouseMove(e) {
    ...
    // highlight the grid the piece is over
    highlightGrid(mousePiece);
}

// if the drop is valid, align the piece up, remove the mousemove and drop elements
